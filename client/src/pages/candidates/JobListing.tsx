...
export default function CandidateJobListing() {
  ...
  const [applyLoadingJobId, setApplyLoadingJobId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    const cachedId = (
      localStorage.getItem(CANDIDATE_ID_STORAGE_KEY) ?? ""
    ).trim();
    ...
    const loadCandidateData = async () => {
      ...
      try {
        const preferredCandidateId =
          candidateId || DEFAULT_RECOMMEND_CANDIDATE_ID;
        let recommendedResponse =
          await getRecommendedJobs(preferredCandidateId);
        ...
        if (
          ...
          recommendedResponse = await getRecommendedJobs(
    ...
    >
      ...
      <main className="mx-auto max-w-7xl px-6 py-6">
        <div className="mb-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600">
          Candidate source: localStorage key{" "}
          <span className="font-semibold">candidate_id</span>
        </div>
          ...
          {recommendedLoading ? (
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {recommendedJobs.map((job) => {
                ...
                const requiredSkills = (job.required_skills ?? []).map(
                  toSkillLabel,
                );
                const preferredSkills = (job.preferred_skills ?? []).map(
                  toSkillLabel,
                );
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-4 flex items-center justify-between gap-3 border-t border-slate-200 pt-3">
                      <span className="text-xs text-slate-500">
                        {applicantsCountByJobId[job.job_id] ??
